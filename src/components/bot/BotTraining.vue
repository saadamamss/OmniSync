...
import { ref } from 'vue';
import { BrainCircuit, Info, Plus, Trash2, ChevronDown, ChevronUp, FileUp, Loader2, RefreshCw, Globe, Eraser, CheckCircle2 } from 'lucide-vue-next';
...
const emit = defineEmits(['update:chunks', 'add-chunk', 'remove-chunk', 'bulk-add', 'refresh-chunks', 'update:websiteUrl']);
...
const removeChunk = async (index: number) => {
  const confirmed = await askConfirm('Remove Topic?', 'Are you sure you want to remove this knowledge topic?');
  if (confirmed) {
...
const clearAllChunks = async () => {
  const confirmed = await askConfirm('Clear All?', 'Are you sure you want to clear ALL knowledge topics? This cannot be undone.');
  if (confirmed) {
    emit('update:chunks', []);
  }
};
...
const handleSyncWebsite = async () => {
  ...
  try {
    ...
    showToast(`Sync Complete! Scanned ${data.pages_scanned} pages.`, 'success');
    emit('refresh-chunks');
  } catch (e: any) {
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="sourceType === 'website'" class="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-xs text-gray-400">Keep your AI updated with your site content.</p>
          </div>
        </div>
        <button 
      ...
      <div class="space-y-2">
        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">Target Website URL</label>
        <input 
          :value="websiteUrl"
          @input="$emit('update:websiteUrl', ($event.target as HTMLInputElement).value)"
          type="url" 
        ...
        />
        <div class="flex items-center mt-3 text-[10px] text-gray-400 bg-gray-50 p-3 rounded-xl">
          <Info class="w-3 h-3 mr-2 text-blue-500" />
          Clicking "Sync Now" will re-scan this URL and common pages to update the knowledge base.
        </div>
      </div>
    </div>
    ...
    <div class="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 space-y-4 sm:space-y-0">
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-xs text-gray-400">{{ chunks.length }} topics indexed</p>
          </div>
        </div>
        <div class="flex items-center space-x-2">
          ...
          <input ref="bulkFileInput" type="file" accept=".txt" class="hidden" @change="handleBulkUpload" />
      ...
      <!-- Chunks List -->
      <div class="space-y-4">
        <div v-if="chunks.length === 0" class="text-center py-16 border-2 border-dashed border-gray-100 rounded-[2rem] bg-gray-50/30">
          ...
          <p class="text-gray-400 text-sm font-medium">No knowledge topics found.</p>
          <p class="text-xs text-gray-300 mt-1">Add some manually or sync your website to train your AI.</p>
        </div>

        <div v-for="(chunk, index) in chunks" :key="index" 
          class="border border-gray-100 rounded-2xl overflow-hidden transition-all"
          :class="chunk.isExpanded ? 'ring-2 ring-orange-500/10 border-orange-200 shadow-lg' : 'hover:border-gray-200'"
        >
          <div 
          ...
          >
            <div class="flex items-center space-x-3 flex-1">
              <div class="w-8 h-8 bg-gray-50 rounded-lg flex items-center justify-center text-xs font-bold text-gray-400">
                {{ index + 1 }}
              </div>
              <input 
                v-model="chunk.title" 
                @click.stop
                @input="updateChunk(index, 'title', ($event.target as HTMLInputElement).value)"
            ...
            <div class="flex items-center space-x-2">
              ...
              <component :is="chunk.isExpanded ? ChevronUp : ChevronDown" class="w-5 h-5 text-gray-400" />
            </div>
          </div>
          ...
          <div v-if="chunk.isExpanded" class="p-4 pt-0 bg-white">
            <textarea
              ...
              @input="updateChunk(index, 'content', ($event.target as HTMLTextAreaElement).value)"
              rows="6"
