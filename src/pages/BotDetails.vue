...
const fetchBotData = async () => {
  ...
  if (botData.settings.source_value === undefined)
    botData.settings.source_value = "";
  bot.value = botData;
...
const checkForChanges = () => {
  ...
  const botChanged =
    JSON.stringify(bot.value) !== JSON.stringify(originalBot.value);
  const chunksChanged =
    JSON.stringify(chunks.value) !== JSON.stringify(originalChunks.value);
...
const handleSave = async () => {
  ...
  const { error: botError } = await supabase
    .from("chatbots")
    .update({ name: bot.value.name, settings: bot.value.settings })
    .eq("id", bot.value.id);
  ...
  await supabase
    ...
    .delete()
    .eq("chatbot_id", bot.value.id);
  const validChunks = chunks.value
...
const toggleBotStatus = async () => {
  ...
  if (!bot.value.is_active) {
    ...
    const limit = limits[profile?.plan || 'starter'];
    ...
    if ((count || 0) >= limit) {
      showToast(`Your plan allows only ${limit} active bot(s). Deactivate another bot first.`, 'error');
      return;
    }
  }

  togglingStatus.value = true;
  const newStatus = !bot.value.is_active;
  ...
  if (!error) {
    bot.value.is_active = newStatus;
    showToast(newStatus ? 'Bot activated successfully!' : 'Bot deactivated.', 'success');
  }
...
<template>
  <DashboardLayout>
    ...
    <div v-else-if="bot" class="max-w-6xl mx-auto">
      <!-- Header -->
      <div
        class="flex flex-col md:flex-row md:items-center justify-between mb-8 space-y-4 md:space-y-0"
      >
        <div class="flex items-center space-x-4">
          <button
            @click="router.push('/dashboard/bots')"
          ...
          <div>
            ...
            <div class="flex items-center space-x-2 mt-1">
              ...
              <span
                ...
                class="flex items-center text-xs px-2 py-0.5 bg-orange-50 text-orange-600 rounded-full font-bold"
              >
        ...
        <div class="flex items-center space-x-3">
          ...
          <button
            ...
            :class="[
              'px-5 py-3 rounded-2xl font-bold flex items-center transition-all border-2',
              bot.is_active
                ? 'border-green-200 bg-green-50 text-green-700 hover:bg-red-50 hover:border-red-200 hover:text-red-600'
                : 'border-gray-200 bg-gray-50 text-gray-500 hover:bg-green-50 hover:border-green-200 hover:text-green-600'
            ]"
          >
            <Loader2 v-if="togglingStatus" class="w-4 h-4 mr-2 animate-spin" />
            <span v-else class="w-2 h-2 rounded-full mr-2"
              :class="bot.is_active ? 'bg-green-500' : 'bg-gray-400'"
            ></span>
            {{ bot.is_active ? 'Active — Click to Pause' : 'Paused — Click to Activate' }}
          </button>
          ...
          <button
            @click="handleSave"
            :disabled="saving"
            class="bg-gray-900 text-white px-8 py-3 rounded-2xl font-bold flex items-center hover:bg-gray-800 transition-all disabled:opacity-50 shadow-lg shadow-gray-200"
          >
      ...
      <!-- Tabs -->
      <div
        class="flex space-x-1 bg-gray-100 p-1.5 rounded-2xl mb-8 w-fit overflow-x-auto max-w-full"
      >
        <button
          @click="activeTab = 'training'"
          :class="[
            'px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center whitespace-nowrap',
            activeTab === 'training'
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-6">
          ...
          <BotAppearance
            ...
            v-model:settings="bot.settings.appearance"
          />
          <BotConversations
          ...
          <BotPlayground
            ...
            :botId="bot.id"
            :appearance="bot.settings.appearance"
          />
